    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] font-sans text-[#1a2e3b] print:bg-white text-base">
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8 flex flex-col lg:flex-row gap-8 items-start relative print:block print:w-full print:max-w-none print:p-0">
                ...
                {(step === 2 || step === 3) && <div className="w-full lg:w-[320px] xl:w-[350px] shrink-0 print:hidden mt-8 lg:mt-0"><div className="sticky top-24 lg:top-28"><Card className="border-t-4 border-t-[#f2ae60] shadow-2xl"><CardHeader className="bg-slate-50 border-b"><CardTitle className="flex gap-2 items-center text-[#2b546e]"><Backpack className="text-[#f2ae60] w-5 h-5" /> Tu Mochila <Badge className="bg-[#2b546e]">{mochila.length}</Badge></CardTitle></CardHeader><CardContent className="p-0"><ScrollArea className="max-h-[300px] lg:h-[calc(100vh-300px)] px-4 py-4">{mochila.map(m => <div key={m.id} className="bg-white p-4 rounded-lg border mb-3 relative group shadow-sm hover:shadow-md transition-all"><button onClick={() => toggleMochila(m)} className="absolute -top-2 -right-2 bg-white rounded-full p-1 border text-slate-300 hover:text-red-500 shadow-sm transition-opacity lg:opacity-0 lg:group-hover:opacity-100"><X className="w-4 h-4" /></button><Badge variant="outline" className="mb-2 text-[10px] text-[#2b546e] border-[#2b546e]">{m.codigo}</Badge><p className="text-sm text-slate-700 line-clamp-3 leading-snug">{m.descripcion}</p></div>)}</ScrollArea></CardContent><CardFooter className="bg-slate-50 p-4">{step === 2 ? <Button onClick={() => setStep(3)} disabled={mochila.length === 0} className="w-full bg-[#2b546e] text-white font-bold h-12 text-base">Configurar Estrategia</Button> : <Button variant="outline" onClick={() => setStep(2)} className="w-full h-12 text-slate-600 font-bold border-2">Agregar más OAs</Button>}</CardFooter></Card></div></div>}
