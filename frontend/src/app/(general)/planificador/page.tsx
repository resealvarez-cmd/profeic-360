...
export default function PlanificadorWizard() {
    ...
    const handleGenerar = async () => {
        setLoading(true);
        try {
            const res = await fetch('https://profeic-backend-484019506864.us-central1.run.app/api/generate', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ nivel, asignatura: Array.from(new Set(mochila.map(m => m.asignatura))).join(" + "), oas_mochila: mochila.map(oa => `[${oa.asignatura}] ${oa.codigo}: ${oa.descripcion}`), valor_panel: valorSeleccionado, actitud_especifica: actitudesSeleccionadas.join(", "), medio_usuario: medioDidactico, num_clases: numClases[0], modo_distribucion: modoDistribucion })
            });
            const data = await res.json();
            if (data.result) { setResultado(data.result); setStep(4); setTimeout(() => { confetti({ particleCount: 180, spread: 100, origin: { y: 0.6 } }); }, 500); }
        } catch (e) { console.error(e); alert("Error de conexión"); } finally { setLoading(false); }
    };
    ...
    const handleDownloadDocx = async () => {
        ...
        try {
            ...
            const response = await fetch("https://profeic-backend-484019506864.us-central1.run.app/export/planificacion-docx", {
                method: "POST",
    ...
    const getStepClassName = (s: number) => { if (step === s) return "bg-[#f2ae60] w-8"; if (step > s) return "bg-[#2b546e] w-4"; return "bg-slate-200 w-2"; };
        ...
        <div className="min-h-screen bg-[#f8fafc] font-sans text-[#1a2e3b] print:bg-white text-base">
            ...
            <header className="bg-white border-b sticky top-0 z-50 shadow-sm/50 backdrop-blur-md bg-white/90 print:hidden">
                <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        {/* AQUÍ ESTABA EL PROBLEMA: Le quitamos el style='auto' y forzamos w-10 h-10 */}
                        <Image 
                            src="/logo.png" 
                            alt="Logo" 
                            width={40} 
            ...
            <main className="max-w-7xl mx-auto px-6 py-8 flex gap-8 items-start relative print:block print:w-full print:max-w-none print:p-0">
                <div className="flex-1 min-w-0 print:w-full">
                    ...
                    {step === 2 && <div className="space-y-6 animate-in slide-in-from-right-8"><div className="flex items-end justify-between"><div><Button variant="ghost" onClick={() => setStep(1)} className="-ml-4 text-slate-400 hover:text-[#2b546e]"><ArrowLeft className="w-4 h-4 mr-1" /> Volver</Button><h2 className="text-3xl font-bold text-[#2b546e]">Objetivos</h2></div><Input placeholder="Buscar clave..." value={busquedaOA} onChange={e => setBusquedaOA(e.target.value)} className="w-80 bg-white shadow-sm border-slate-200" /></div>
                ...
                {step === 4 && resultado && (
                    <div className="w-full max-w-6xl mx-auto animate-in fade-in pb-20 print:max-w-none">
                        <div className="sticky top-24 z-40 mb-8 flex justify-between items-center bg-white/90 backdrop-blur p-4 rounded-2xl border shadow-lg print:hidden">
                            <Button variant="ghost" onClick={() => setStep(3)} className="text-slate-500 hover:text-[#2b546e]"><ArrowLeft className="w-4 h-4 mr-2" /> Editar Configuración</Button>
                            <div className="flex gap-3">
                                ...
                                <Button variant="outline" onClick={handleCopyAll} className="text-[#2b546e] border-[#2b546e] hover:bg-blue-50"><Copy className="w-4 h-4 mr-2" /> Copiar HTML</Button>
                            </div>
                        </div>
