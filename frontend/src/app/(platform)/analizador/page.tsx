...
export default function AnalizadorPage() {
    ...
    const [contextoEscenario, setContextoEscenario] = useState<"practicado" | "inedito" | "">("");

    const handleAnalyze = async () => {
        if (!objetivo.trim() || !instrumento.trim()) return;
        if (!contextoEscenario) {
            toast.error("Por favor, selecciona si el contenido fue practicado o es inédito.");
            return;
        }
        setIsAnalyzing(true);
        setResult(null);
        ...
        try {
            ...
            try {
                response = await fetch(`${API_URL}/api/v1/analizador/audit`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        objetivo_aprendizaje: objetivo,
                        texto_evaluacion: instrumento,
                        contexto_escenario: contextoEscenario
                    }),
                    ...
                    {inputCollapsed ? (
                        ...
                        <div className="p-5">
                            ...
                            <div className="space-y-4">
                                ...
                                {/* --- RADIO BUTTONS DE CONTEXTO --- */}
                                <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Contexto de Aplicación</p>

                                    <div
                                        onClick={() => setContextoEscenario("practicado")}
                                        className={cn(
                                            "flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all",
                                            contextoEscenario === "practicado"
                                                ? "bg-white border-[#2b546e] shadow-sm ring-1 ring-[#2b546e]/20"
                                    ...
                                    >
                                        <div className={cn(
                                            ...
                                            contextoEscenario === "practicado" ? "border-[#2b546e]" : "border-slate-300"
                                        )}>
                                            {contextoEscenario === "practicado" && <div className="w-2 h-2 bg-[#2b546e] rounded-full" />}
                                        </div>
                                        <div>
                                            <p className="text-xs font-bold text-[#2b546e]">Opción A</p>
                                            <p className="text-[10px] text-slate-500 leading-tight mt-0.5">
                                                El contenido evaluado fue practicado en clases con ejercicios similares.
                                            </p>
                                        </div>
                                    </div>

                                    <div
                                        onClick={() => setContextoEscenario("inedito")}
                                        className={cn(
                                            "flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all",
                                            contextoEscenario === "inedito"
                                                ? "bg-white border-[#f2ae60] shadow-sm ring-1 ring-[#f2ae60]/20"
                                                : "bg-transparent border-slate-200 hover:border-slate-300"
                                        )}
                                    >
                                        <div className={cn(
                                            "mt-1 w-4 h-4 rounded-full border-2 flex items-center justify-center",
                                            contextoEscenario === "inedito" ? "border-[#f2ae60]" : "border-slate-300"
                                        )}>
                                            {contextoEscenario === "inedito" && <div className="w-2 h-2 bg-[#f2ae60] rounded-full" />}
                                        </div>
                                        <div>
                                            <p className="text-xs font-bold text-[#7c4d12]">Opción B</p>
                                            <p className="text-[10px] text-slate-500 leading-tight mt-0.5">
                                                Esta evaluación contiene escenarios, problemas o textos INÉDITOS para el estudiante.
                                            </p>
                                        </div>
                                    </div>
                                </div>

                                <Button onClick={handleAnalyze} disabled={isAnalyzing || !objetivo || !instrumento || !contextoEscenario} className="w-full bg-[#2b546e] hover:bg-[#1e3b4e] text-white font-bold h-12 shadow-lg shadow-blue-900/10">
                                    <Sparkles className="w-4 h-4 mr-2" /> {result ? "Re-Auditar Instrumento" : "Comenzar Auditoría"}
                                </Button>
                            </div>
                        </div>
                    )}
